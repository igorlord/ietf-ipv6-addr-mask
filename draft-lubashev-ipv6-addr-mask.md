...
abbrev: v6AddrMask
docname: draft-lubashev-ipv6-addr-mask-00
date: {DATE}
category: info
...
With the move to IPv6, the primary mechanism for matching on addresses
...
semantics to bit strings within addresses beyond the prefix.  Here we
define a syntax for specifying groupings (matching rules) of IPv6
addresses where a set of less-significant bits have a particular
value.  For example, 2001:dba8::1234/ffff:ffff::ffff would match on
any addresses starting with 2001:dba8::/32 and ending with the last 16
bits being ::1234.
...
be used to identify a single host and its subnet.
...
and a prefix length. For example: 198.51.100.4/24 or
2001:db8::1234/32.
...
1. Allocating a block of addresses to each host and using the least
   ...
   information. These operators may need a way to express a rule that
   applies to all traffic of a particular traffic class.  For example,
   an operator may route an address prefix to a physical server or
   virtual machine host and then use less significant bits to indicate
   an application, virtual machine guest, or TLS certificate.
...
2. Using "middle" bits of IPv6 addresses to specify traffic class or
   ...
   identify sites and individual hosts. While only applicable with an
   administrative scope, these operators need a way to express both
   rules that apply to particular hosts as well as to particular
   traffic classes.  For example, a corporate network environment may
   use a prefix per-site but then use the next bits to indicate
   whether a subnet is a lab, guest wireless, production environment,
   or other.
...
This RFC extends prefix-length notation of {{RFC4632}} in a way that
is reminiscent of the IPv4 netmask notation of {{RFC0950}}. The
proposed notation would allow specifying IPv6 mask instead of the
prefix length after "/".  For example: 2001:db8::1234/ffff:ffff::ffff.
...
scope.  For example, specifying all addresses ending with ::1234
within 2001:db8::/32 can be specified as
...
administrative domains.  As such, care must be taken when applying any
semantic meaning to bits in addresses.  Guidance for this
